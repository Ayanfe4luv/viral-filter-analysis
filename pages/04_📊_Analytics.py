# -*- coding: utf-8 -*-
"""
pages/04_📊_Analytics.py — Vectorized Visualizations + Extended Chart Suite
...
def _make_violin(df: pd.DataFrame, group_col: str, scheme) -> go.Figure:
    ...
    fig.update_layout(
        **_LAYOUT, height=500,
        ...
        xaxis_title=group_col if group_col in df.columns else "",
    )
    return fig
...
def _make_bubble(df: pd.DataFrame, interval_code: str,
    ...
    fig.update_xaxes(title=T("analytics_period_label"), tickangle=45)
    fig.update_yaxes(title=y_field)
...
if "an_fig" in st.session_state:
    ...
    with dl_right:
        st.caption(
            f"\U0001f4be {T('analytics_download_html_help')}"
        )
