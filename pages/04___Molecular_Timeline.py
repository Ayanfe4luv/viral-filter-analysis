...
_batch_mode = False  # default — overridden below if multi-file scope is active

if len(_tl_contrib) > 1:
    _file_names = [rf["name"] for rf in _tl_contrib]

    _tl_scope_sel = st.multiselect(
        T("timeline_scope_label"),
        options=_file_names,
        default=st.session_state.get("tl_scope_sel", _file_names[:1]),
        key="tl_file_scope",
        help=T("timeline_scope_help")
        + " · Select **multiple files** to run batch Timeline analysis across all of them.",
        placeholder="Select one or more files…",
    )
    st.session_state["tl_scope_sel"] = _tl_scope_sel

    _batch_mode = len(_tl_scope_sel) > 1

    if not _tl_scope_sel:
        # Nothing selected → fall back to merged active dataset
        st.caption(T("timeline_scope_all_caption", n=len(_tl_contrib)))
    elif _batch_mode:
        st.info(
            f"📦 **Batch mode — {len(_tl_scope_sel)} files selected.** "
            "The same Timeline Configuration below will be applied to every selected file. "
            "Scroll past Phase 3 to view and download per-file results in separate tabs.",
            icon=None,
        )
    else:
        # Single file — scope as before
        _tl_scope = _tl_scope_sel[0]
...
with st.expander(f"🔍 {T('timeline_diagnostics_header')}", expanded=True):
    ...
    if unique_hashes < total_seqs:
        ...
        with _ctrl_sp:
            ...
            st.caption(
                "⚠️ **This slider adjusts the chart and table view only.** "
                "All clusters are always included in the CSV downloads below — "
                "sliding left never removes data from exports."
            )
        ...
        try:
            ...
            with _col_sun:
                ...
                if _right_view == _view_tbl:
                    ...
                    # ── Size Bucket + Epidemiological Implication ──────────────
                    _IMPL_MAP_TBL = {
                        T("timeline_cluster_singletons"): T("timeline_cluster_implication_singleton"),
                        T("timeline_cluster_small"):      T("timeline_cluster_implication_small"),
                        T("timeline_cluster_medium"):     T("timeline_cluster_implication_medium"),
                        T("timeline_cluster_large"):      T("timeline_cluster_implication_large"),
                        T("timeline_cluster_major"):      T("timeline_cluster_implication_major"),
                    }
                    _tbl[T("timeline_cluster_size_bucket")] = _tbl["count"].apply(_size_bucket)
                    _tbl[T("timeline_cluster_implication")] = (
                        _tbl[T("timeline_cluster_size_bucket")].map(_IMPL_MAP_TBL)
                    )
                    _tbl_rename = {
                    ...
                    # Per-view export button
                    _pfx_tv = st.session_state.get("export_prefix", "virsift") or "virsift"
                    st.download_button(
                        label="⬇️ Export this Table view as CSV",
                        data=_tbl.to_csv(index=False).encode("utf-8"),
                        file_name=f"{_pfx_tv}_cluster_table_view.csv",
                        mime="text/csv",
                        use_container_width=True,
                        help="Exports exactly what is shown in the table (top-N rows, with Size Bucket and Implication columns).",
                    )
                    ...
                    _pfx_tv = st.session_state.get("export_prefix", "virsift") or "virsift"
                    st.download_button(
                        label="⬇️ Export Treemap data as CSV",
                        data=_sun_agg.to_csv(index=False).encode("utf-8"),
                        file_name=f"{_pfx_tv}_cluster_treemap.csv",
                        mime="text/csv",
                        use_container_width=True,
                        help="Exports the aggregated size-bucket breakdown driving this Treemap.",
                    )
                    ...
                    _pfx_tv = st.session_state.get("export_prefix", "virsift") or "virsift"
                    st.download_button(
                        label="⬇️ Export Sunburst data as CSV",
                        data=_sun_agg.to_csv(index=False).encode("utf-8"),
                        file_name=f"{_pfx_tv}_cluster_sunburst.csv",
                        mime="text/csv",
                        use_container_width=True,
                        help="Exports the aggregated size-bucket breakdown driving this Sunburst chart.",
                    )
...
# ─────────────────────────────────────────────────────────────────────────────
# BATCH MODE — Run analysis across all selected files, show results in tabs
# ─────────────────────────────────────────────────────────────────────────────
if _batch_mode:
    st.divider()
    st.subheader("📦 Batch Timeline Results")
    st.caption(
        "Each tab below shows the cluster summary and curated FASTA download for one file. "
        "Configuration (min cluster size, representative logic, max per month) is shared across all files."
    )

    _batch_scope_sel = st.session_state.get("tl_scope_sel", [])
    _batch_files_data = [
        (rf["name"], pd.DataFrame(rf["parsed"]))
        for rf in _tl_contrib
        if rf["name"] in _batch_scope_sel
    ]

    if _batch_files_data:
        _batch_tabs = st.tabs([name for name, _ in _batch_files_data])

        for _tab, (_fname, _fdf) in zip(_batch_tabs, _batch_files_data):
            with _tab:
                # Ensure sequence_hash
                if "sequence_hash" not in _fdf.columns and "sequence" in _fdf.columns:
                    _fdf = _fdf.copy()
                    _fdf["sequence_hash"] = (
                        _fdf["sequence"].fillna("")
                        .apply(lambda s: hashlib.md5(s.upper().encode()).hexdigest()[:12])
                    )

                if _fdf.empty or "sequence_hash" not in _fdf.columns:
                    st.warning(f"No parseable data in {_fname}.")
                    continue

                _f_total   = len(_fdf)
                _f_unique  = _fdf["sequence_hash"].nunique()
                _f_clusters = _build_clusters(_fdf, min_cluster)

                fc1, fc2, fc3 = st.columns(3)
                fc1.metric("Sequences", f"{_f_total:,}")
                fc2.metric("Unique Clones", f"{_f_unique:,}")
                fc3.metric(f"Clusters ≥ {min_cluster}", f"{len(_f_clusters):,}")

                # Cluster summary table with size bucket + implication
                if not _f_clusters.empty:
                    _f_sum = _f_clusters.copy()
                    _f_sum["Size Bucket"] = _f_sum["count"].apply(_size_bucket)
                    _BATCH_IMPL_MAP = {
                        T("timeline_cluster_singletons"): T("timeline_cluster_implication_singleton"),
                        T("timeline_cluster_small"):      T("timeline_cluster_implication_small"),
                        T("timeline_cluster_medium"):     T("timeline_cluster_implication_medium"),
                    ...
                    _f_sum["Epidemiological Implication"] = _f_sum["Size Bucket"].map(_BATCH_IMPL_MAP)
                    st.dataframe(_f_sum, use_container_width=True, hide_index=True, height=320)

                    _pfx_b = st.session_state.get("export_prefix", "virsift") or "virsift"
                    _safe_fname = _fname.replace(".fasta", "").replace(".fa", "").replace(" ", "_")
                    b1, b2 = st.columns(2)

                    with b1:
                        st.download_button(
                            label=f"⬇️ Cluster CSV — {_fname}",
                            data=_f_sum.drop(columns=["sequence_hash"], errors="ignore")
                                       .to_csv(index=False).encode("utf-8"),
                            file_name=f"{_pfx_b}_{_safe_fname}_clusters.csv",
                            mime="text/csv",
                            use_container_width=True,
                        )

                    with b2:
                        # Auto-curate: keep 1 rep per cluster per month (earliest)
                        if "sequence" in _fdf.columns and "collection_date" in _fdf.columns:
                            try:
                                from utils.gisaid_parser import convert_df_to_fasta
                                _hashes_in = set(_f_clusters["sequence_hash"])
                                _fdf_filtered = _fdf[_fdf["sequence_hash"].isin(_hashes_in)].copy()
                                _fdf_filtered["_month"] = pd.to_datetime(
                                    _fdf_filtered["collection_date"], errors="coerce"
                                ).dt.to_period("M").astype(str)
                                _batch_curated = (
                                    _fdf_filtered
                                    .sort_values("collection_date")
                                    .drop_duplicates(subset=["sequence_hash", "_month"])
                                    .drop(columns=["_month"])
                                )
                                _fasta_out = convert_df_to_fasta(_batch_curated)
                                st.download_button(
                                    label=f"⬇️ Curated FASTA — {_fname}",
                                    data=_fasta_out if isinstance(_fasta_out, bytes)
                                         else _fasta_out.encode("utf-8"),
                                    file_name=f"{_pfx_b}_{_safe_fname}_curated.fasta",
                                    mime="text/plain",
                                    use_container_width=True,
                                )
                            except Exception:
                                st.info("Install FASTA export utils to enable FASTA download.")
                        else:
                            st.info("No sequence column — FASTA download unavailable for this file.")
                else:
                    st.info(f"No clusters with ≥ {min_cluster} sequences found in {_fname}.")

# In batch mode: skip the interactive single-file Phase 3 / Phase 4 matrix UI
if _batch_mode:
    st.divider()
    try:
        _tl_n1b, _tl_n2b = st.columns(2)
        _tl_n1b.page_link("pages/03_🔬_Sequence_Refinery.py",
                          label=f"← 🔬 {T('nav_refinery')}",
                          use_container_width=True)
        _tl_n2b.page_link("pages/05_📊_Analytics.py",
...
# ─────────────────────────────────────────────────────────────────────────────
# PHASE 3 — Interactive Timeline Matrix  (single-file mode only)
# ─────────────────────────────────────────────────────────────────────────────
