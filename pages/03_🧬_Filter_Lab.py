# -*- coding: utf-8 -*-
"""
pages/03_🧬_Filter_Lab.py — Pre-Processing, Filtering & HITL Sampling
...
c1.metric("Active dataset",  f"{len(_active_df):,}")
c2.metric("After filters",   f"{len(current):,}")
delta = len(current) - len(_active_df)
c3.metric("\u0394 sequences", f"{delta:+,}", delta_color="inverse")

if not st.session_state.get("filtered_df", pd.DataFrame()).empty:
    if st.button("\u21a9 Reset all filters (restore active dataset)", type="secondary"):
...
with st.expander(f"🔬 {T('filter_quality_header')}", expanded=True):
    ...
    if st.button(T("filter_apply_quality"), type="primary", use_container_width=True):
        ...
        st.success(
            f"Quality filters applied — {len(result):,} sequences retained "
            f"({removed:,} removed)."
        )
        st.rerun()
...
with st.expander(f"🏷 {T('filter_header_comp_header')}"):
    ...
    if not available_fields:
        ...
        st.markdown("**Add a filter rule:**")
        ra, rb, rc, rd = st.columns([2, 2, 3, 1])
        with ra:
            new_field = st.selectbox(
                "Field", options=list(available_fields.keys()), key="new_field"
            )
        with rb:
            new_op = st.selectbox(
                "Operator",
        ...
        with rc:
            new_val = st.text_input(
                "Value (comma-separated for in_list / date_range)",
                key="new_val",
                placeholder="e.g. H3N2  or  H3N2,H1N1  or  2022-01-01,2023-12-31",
            )
        with rd:
            st.write("")
            st.write("")
                ...
                if new_val.strip():
                    ...
                    if new_op == "in_list":
                        ...
                        try:
                            ...
                            st.error("date_range value must be: start,end (YYYY-MM-DD)")
                            value = None
        ...
        if rules:
            ...
            for i, rule in enumerate(rules):
                ...
                if col_del.button("Remove", key=f"del_rule_{i}"):
                    rules.pop(i)
                    st.session_state["filter_rules"] = rules
                    st.rerun()
            ...
            if st.button(T("filter_apply_header"), type="primary", use_container_width=True):
                ...
                st.success(
                    f"Header filters applied — {len(result):,} sequences retained."
                )
                st.rerun()
        else:
            st.info("No rules added yet. Add at least one rule above, then apply.")
...
with st.expander(f"🔑 {T('filter_accession_header')}"):
    ...
    if st.button(T("filter_apply_accession"), disabled=not acc_text.strip(),
        ...
        st.success(
            f"Accession filter applied — {len(result):,} sequences matched "
            f"from {len(acc_list)} accessions provided."
        )
        st.rerun()
...
with st.expander(f"🧠 {T('hitl_header')}"):
    ...
    st.info(
        f"**Dataset lifespan category:** {category}  "
...
if not filtered_df.empty:
    ...
    if filter_logs:
        methodology = {
            "tool":    "Vir-Seq-Sift International v2.1",
            "filters": filter_logs,
            "result":  {
